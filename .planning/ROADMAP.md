...
RedClass is built bottom-up across **5 platforms** (Windows / macOS / Linux / Android / iOS) from a single Flutter codebase. The architecture has one explicit platform branch: **LLM parsing is desktop-only** (Windows/macOS/Linux); **mobile devices (Android/iOS) consume JSON files exported from a desktop**. This is documented as an explicit decision in PROJECT.md and surfaced in the UI as a platform-conditional import page.

Phases execute bottom-up: stable Flutter + SQLite foundation → read-only file import → desktop-only LLM integration → three-mode wrong-question ledger → JSON export/import for cross-device transfer → multiple-choice + bookmarks + stats → UX polish + diagnostics → five-platform packaging + cross-platform verification. By Phase 8 the user can install on any of the 5 platforms, drop a teacher's `.docx` on a desktop, export JSON, sideload JSON to a phone, and study within minutes — all offline.
...
- [ ] **Phase 1: Foundation & Persistence** - Flutter project skeleton (5 platforms), drift schema, go_router, Material 3 theme, PathResolver
...
- [ ] **Phase 3: Desktop LLM Integration** - `LlmClient` abstraction (Stub + HTTP), model picker, GBNF-constrained JSON parsing — **desktop-only**, gated on `Platform.isDesktop`
- [ ] **Phase 4: Quiz Core & Wrong-Question Ledger** - Quiz screen, single-choice grading, three review modes, shared ledger state machine
- [ ] **Phase 5: JSON Cross-Device Transfer + Multiple-Choice + Bookmarks + Statistics** - JSON export (desktop), JSON import (all platforms), multi-choice, bookmarks, stats, **platform-conditional import UI**
- [ ] **Phase 6: UX Polish & Diagnostics** - UI alignment with ui-ux-pro-max, session state recovery, diagnostic-pack export
- [ ] **Phase 7: Five-Platform Packaging & Verification** - Windows + macOS + Linux + Android + iOS builds, real-device LLM tuning on desktop, real-device JSON import on mobile

## Phase Details

### Phase 1: Foundation & Persistence
**Goal**: Bootstrap a runnable Flutter app on all 5 platforms with a stable drift schema, cross-platform path resolution, and a navigation skeleton.
**Depends on**: Nothing (first phase)
**Requirements**: IMP-05, STOR-01, STOR-02, PLT-06, PLT-07, UI-02
**Success Criteria** (what must be TRUE):
  1. `flutter run -d windows` and `flutter run -d macos` and `flutter run -d linux` and `flutter run -d <android>` and `flutter run -d <ios>` all launch a Material 3 home screen with placeholder bank list and three review-mode entries
  2. drift database initializes at `getApplicationSupportDirectory()/redclass.db` on all five platforms and survives app restart
  3. Schema migration scaffolding exists (version 1 baseline + `onUpgrade` chain) so future schema changes won't lose data
  4. `PathResolver` is the single source of truth for db, model, JSON-import, and JSON-export paths; no other file in the repo calls `path_provider` directly
...
Plans:
- [ ] 01-01: Create Flutter project (`flutter create --platforms=windows,macos,linux,android,ios`) + pubspec deps
...
- [ ] 01-06: Cross-platform smoke test (`flutter test` + manual run on each platform)

**UI hint**: yes (Material 3 home screen, placeholder routes on all 5 platforms)
**Research flag**: no (standard Flutter + drift setup)
**Pitfalls addressed**: PITFALL 3 (SQLite FFI platform quirks — get PathResolver right from day 1, includes macOS sandbox + Linux file permissions), PITFALL 7 (lifecycle — state commit on every write baked in)
**Dependencies**: none (first phase)

### Phase 2: Desktop File Import Pipeline
**Goal**: Desktop user can pick a `.docx` or `.pdf` from local filesystem, app extracts plain text and parses questions via a heuristic/regex parser, then shows a preview screen where the user can edit/delete before committing to the database. Mobile platforms see an import page stub but the `.docx`/`.pdf` entries are hidden.
**Depends on**: Phase 1
...
**Success Criteria** (what must be TRUE):
  ...
  6. Mobile platforms' import page renders the `.json`-only entry; `.docx`/`.pdf` buttons are not visible (gated on `Platform.isDesktop`)
...
- [ ] 02-08: Platform-conditional import page UI (desktop: `.docx`/`.pdf`/`.json`; mobile: `.json` only) using `Platform.isDesktop` / `Platform.isIOS` / `Platform.isAndroid`
...
**Goal**: Replace the heuristic parser with an on-device LLM-driven parser via a swappable `LlmClient` abstraction. Desktop-only; mobile platforms' `LlmClient` provider is stubbed to throw `UnsupportedError`. Ship Stub + HTTP implementations first; FFI binding evaluated as a spike.
**Depends on**: Phase 2
**Requirements**: IMP-03
**Success Criteria** (what must be TRUE):
  ...
  2. `LlmClient` Riverpod provider is **gated on `Platform.isDesktop`** — on Android/iOS the provider throws `UnsupportedError("LLM is desktop-only; use JSON import on mobile")`
...
- [ ] 03-07: **Research spike** (1 week): prototype llama.cpp FFI shim on Windows + macOS + Linux + Android (low-end); go/no-go decision
...
**Research flag**: **HIGH** — Phase 3 genuinely needs a 1-week FFI spike before detailed planning; plan with a fallback (HTTP-only) if the spike fails. iOS FFI has additional complexity (sandbox + Metal) and can be skipped initially.
...
**Goal**: Ship a runnable quiz loop with single-choice questions, all three review modes wired to a shared wrong-question ledger via an atomic state machine. Platform-agnostic — works identically on all 5 platforms once questions are loaded.
**Depends on**: Phase 3
**Requirements**: QST-01, REV-01, REV-02, REV-03, REV-04, REV-05, REV-06, STAT-01, UI-03
**Success Criteria** (what must be TRUE):
  1. User can enter 乱序抽题 on any of the 5 platforms, see a randomly drawn single-choice question, submit an answer, and get immediate correct/incorrect feedback
...
- [ ] 04-09: Cross-platform smoke test of quiz loop on all 5 platforms
...
**Goal**: Layer in JSON export (desktop) and JSON import (all platforms) as the cross-device transfer protocol. Add remaining table-stakes features: multiple-choice rendering/grading, bookmarks, statistics with per-bank aggregation.
**Depends on**: Phase 4
**Requirements**: IMP-06, IMP-07, PLT-08, QST-02, BMK-01, BMK-02, STAT-02
**Success Criteria** (what must be TRUE):
  ...
  2. Any of the 5 platforms can import a previously-exported `.json` and get a fully functional bank (questions + correct answers + metadata)
...
- [ ] 05-03: `JsonImporter` (all platforms) — parses JSON, validates against schema, creates new `QuestionBank` + `Question` rows
...
**Requirements**: UI-01
**Success Criteria** (what must be TRUE):
  1. UI is visually consistent with `ui-ux-pro-max` reference (typography, color, spacing, focus mode) across all 5 platforms
...
### Phase 7: Five-Platform Packaging & Cross-Platform Verification
**Goal**: Produce shippable artifacts for all 5 platforms; validate on-device LLM behavior on real desktop hardware and JSON import flow on real mobile hardware.
**Depends on**: Phase 6
...
**Success Criteria** (what must be TRUE):
  ...
  2. `flutter build macos --release` produces a `.app` bundle (and `.dmg` if signed)
  3. `flutter build linux --release` produces a portable executable + `.AppImage`
  4. `flutter build apk --split-per-abi` produces `arm64-v8a` (mandatory) and `x86_64` (optional) APKs
  5. `flutter build ios --release` produces an `.ipa` (requires Apple Developer account; archive succeeds at minimum)
  6. JSON round-trip smoke test: export from desktop → sideload to real Android device → import → quiz flow works
  7. JSON round-trip smoke test: export from desktop → AirDrop / Files transfer to real iOS device → import → quiz flow works
  8. README documents install steps for all 5 platforms, model download flow (desktop only), JSON transfer workflow, and troubleshooting section
**Plans**: TBD (estimated 5-7 plans for "standard" granularity)

Plans:
- [ ] 07-01: Windows release build + portable `.exe` packaging + SmartScreen notes
- [ ] 07-02: macOS release build + code-signing notes + `.dmg` packaging
- [ ] 07-03: Linux release build + `.AppImage` + dependency notes (gtk-3, etc.)
- [ ] 07-04: Android APK build with split-per-abi + signing config
- [ ] 07-05: iOS archive build + TestFlight notes (signing flow)
- [ ] 07-06: Desktop on-device LLM validation (Windows + macOS + Linux, capability probe, model picker)
- [ ] 07-07: Mobile JSON-import validation (Android + iOS, real device smoke tests)
- [ ] 07-08: README with install steps, JSON transfer workflow, model download (desktop), troubleshooting

**UI hint**: no (build/CI work; no new screens)
**Research flag**: no
**Pitfalls addressed**: PITFALL 4 (desktop OOM — real-device validation on Windows/macOS/Linux), cross-platform packaging gotchas
...
| 7. Five-Platform Packaging & Verification | 0/8 | Not started | - |

**Summary:**
- Total phases: 7
- Total v1 requirements: 33
- Coverage check: 33/33 mapped (100%) ✓
- Phases with research flag: 2 (Phase 2 — docx samples; Phase 3 — LLM FFI spike)
- Total estimated plans: 54 across all phases
- Platforms: 5 (Windows / macOS / Linux / Android / iOS)
- Platform branches: 2 explicit (LLM desktop-only; JSON import all platforms)
