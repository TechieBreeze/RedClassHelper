...
''';
      ...
      // 多选题检测
      final isMulti =
          q.candidateType == CandidateType.multiChoice ||
...
''';
      ...
      final q = candidates.first;
      final isShortAnswer =
          q.candidateType == CandidateType.shortAnswer ||
...
''';
    ...
    test(
      'strips answer letters from （X）but keeps brackets (inline format)',
      () {
        const input = '''
一国两制的前提是（A）
A. 坚持一个中国原则
...
''';
      ...
      expect(candidates, isNotEmpty);
      expect(candidates.first.options.length, 4);
      expect(candidates.first.options[3], contains('实现中华民族伟大复兴的必由之路'));
    });
...
''';
    ...
    test(
      'options on same line as question number are separated from title',
      () {
        const input = '''
...
''';
      ...
      expect(candidates, isNotEmpty);
      expect(
        candidates.first.options.length,
        4,
