...
void main() {
  ...
  // ═════════════════════════════════════════════════════════════
  test(
    'build() in random mode loads all questions from bank and shuffles',
    () async {
      ...
      final state = await container.read(
        quizSessionControllerProvider('b1', 'random').future,
      );
  ...
  // ═════════════════════════════════════════════════════════════
  test('build() in review mode loads only questions from '
    ...
    final state = await container.read(
      quizSessionControllerProvider('b1', 'review').future,
    );
  ...
  // ═════════════════════════════════════════════════════════════
  test(
    'build() in spotcheck mode loads at most 10 from active ledger',
    () async {
      for (var i = 0; i < 15; i++) {
        await _insertQuestion(
          db: db,
      ...
      final state = await container.read(
        quizSessionControllerProvider('b1', 'spotcheck').future,
      );
  ...
  // ═════════════════════════════════════════════════════════════
  test(
    'submitAnswer() with correct option: status becomes showingFeedback',
    () async {
      await _insertQuestion(
        db: db,
        questionId: 'q1',
      ...
      final controller = container.read(
        quizSessionControllerProvider('b1', 'random').notifier,
      );
  ...
  // ═════════════════════════════════════════════════════════════
  test('submitAnswer() incorrect in random mode: '
    ...
    await container.read(quizSessionControllerProvider('b1', 'random').future);
  ...
  // ═════════════════════════════════════════════════════════════
  test(
    'submitAnswer() correct in review mode calls recordCorrectReview',
    () async {
      await _insertQuestion(
        db: db,
        questionId: 'q1',
      ...
      final controller = container.read(
        quizSessionControllerProvider('b1', 'review').notifier,
      );
      await container.read(
        quizSessionControllerProvider('b1', 'review').future,
      );
      ...
      )..where((e) => e.questionId.equals('q1'))).getSingle();
  ...
  // ═════════════════════════════════════════════════════════════
  test(
    'submitAnswer() in spotcheck mode never calls markWrong or markMastered',
    () async {
      await _insertQuestion(
        db: db,
        questionId: 'q1',
      ...
      final controller = container.read(
        quizSessionControllerProvider('b1', 'spotcheck').notifier,
      );
      await container.read(
        quizSessionControllerProvider('b1', 'spotcheck').future,
      );
  ...
  // ═════════════════════════════════════════════════════════════
  test(
    'advanceToNext() increments currentIndex and detects completion',
    () async {
      await _insertQuestion(
        db: db,
        questionId: 'q1',
  ...
  test('build() with invalid mode string throws ArgumentError', () async {
    ...
    expect(
      () => container.read(quizSessionControllerProvider('b1', 'rando').future),
  ...
  test('Empty question list sets status to complete immediately', () async {
    ...
    final state = await container.read(
      quizSessionControllerProvider('emptyBank', 'random').future,
    );
  ...
  test('build() with non-existent bankId returns error status', () async {
    ...
    final state = await container.read(
      quizSessionControllerProvider('nonexistent', 'random').future,
    );
  ...
  // ═════════════════════════════════════════════════════════════
  test('multi-choice exact match: selecting all correct options '
    ...
    addTearDown(container.dispose);
    final controller = container.read(
      quizSessionControllerProvider('bM1', 'random').notifier,
    );
    await container.read(quizSessionControllerProvider('bM1', 'random').future);
  ...
  // ═════════════════════════════════════════════════════════════
  test('multi-choice exact match: selecting only a subset of '
    ...
    addTearDown(container.dispose);
    final controller = container.read(
      quizSessionControllerProvider('bM2', 'random').notifier,
    );
    await container.read(quizSessionControllerProvider('bM2', 'random').future);
  ...
  // ═════════════════════════════════════════════════════════════
  test('multi-choice exact match: selecting correct options plus '
    ...
    addTearDown(container.dispose);
    final controller = container.read(
      quizSessionControllerProvider('bM3', 'random').notifier,
    );
    await container.read(quizSessionControllerProvider('bM3', 'random').future);
  ...
  // ═════════════════════════════════════════════════════════════
  test('multi-choice exact match: selecting none of the correct '
    ...
    addTearDown(container.dispose);
    final controller = container.read(
      quizSessionControllerProvider('bM4', 'random').notifier,
    );
    await container.read(quizSessionControllerProvider('bM4', 'random').future);
  ...
  // ═════════════════════════════════════════════════════════════
  test('multi-choice wrong answer in random mode adds to '
    ...
    addTearDown(container.dispose);
    final controller = container.read(
      quizSessionControllerProvider('bM5', 'random').notifier,
    );
    await container.read(quizSessionControllerProvider('bM5', 'random').future);
  ...
  // ═════════════════════════════════════════════════════════════
  test('multi-choice answer stores given selection as JSON array '
    ...
    addTearDown(container.dispose);
    final controller = container.read(
      quizSessionControllerProvider('bM6', 'random').notifier,
    );
    await container.read(quizSessionControllerProvider('bM6', 'random').future);
    await controller.submitAnswer(['A', 'C']);
  ...
  // ═════════════════════════════════════════════════════════════
  test('multi-choice correct answer in review mode marks question '
      'as mastered', () async {
    await _insertMultiChoiceQuestion(
      db: db,
      questionId: 'mq7',
    ...
    addTearDown(container.dispose);
    final controller = container.read(
      quizSessionControllerProvider('bM7', 'review').notifier,
    );
    await container.read(quizSessionControllerProvider('bM7', 'review').future);
    ...
    final entry = await (db.select(
      db.wrongLedgerEntries,
    )..where((e) => e.questionId.equals('mq7'))).getSingle();
    expect(entry.masteredAt, isNotNull);
  });
  ...
  test('multi-choice: submitting empty selection scores incorrect', () async {
    ...
    addTearDown(container.dispose);
    final controller = container.read(
      quizSessionControllerProvider('bM8', 'random').notifier,
    );
    await container.read(quizSessionControllerProvider('bM8', 'random').future);
