...
- **D-04:** 预设模型分三级——推荐（Qwen2.5-1.5B Q4_K_M，默认，平衡质量与速度）、快速（Qwen2.5-0.5B Q4_K_M，极速但精度略低）、实验（Qwen2.5-3B Q4_K_M，最高质量需 4GB+ 内存）。每个模型展示名称、大小、内存需求、推荐理由。
- **D-05:** 应用内 HTTP 下载，显示进度条和速度。支持断点续传（HTTP Range 请求）——中断后可从上次位置继续下载。
