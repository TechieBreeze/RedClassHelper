...
const _testFixtures = <String, dynamic>{
  ...
  'truefalse': {
    ...
    'answer': 'A',
    'explanation':
        '图灵测试由艾伦·图灵于1950年提出，用于测试机器是否展现出与人类相当的智能行为。',
  },
};
...
void main() {
  ...
  group('StubLlmClient.parse()', () {
    test('parse() with default text returns singleChoice, answer="B", 4 options',
        () async {
      final client = StubLlmClient(fixtures: _testFixtures);
    ...
    test('parse() with text containing "以下哪些" returns multiChoice, answer="ABC"',
        () async {
      final client = StubLlmClient(fixtures: _testFixtures);
      final candidate =
          await client.parse('以下哪些属于正确的选项？请选择。');
    ...
    test('parse() with text containing "判断对错" returns trueFalse',
        () async {
      final client = StubLlmClient(fixtures: _testFixtures);
    ...
    test('two calls with same rawText produce byte-identical results',
        () async {
      final client = StubLlmClient(fixtures: _testFixtures);
    ...
    test('parse() fixtures are lazy-loaded (first call triggers load)',
        () async {
      final client = StubLlmClient(fixtures: _testFixtures);
    ...
    test('parse() returns non-empty explanation for all fixture types',
        () async {
      final client = StubLlmClient(fixtures: _testFixtures);
      final defaultResult = await client.parse('default');
      final multiResult = await client.parse('以下哪些');
