...
void main() async {
  ...
  for (final file in docxFiles) {
    ...
    final noAns = candidates
        .where((c) => c.answer.isEmpty && c.options.isNotEmpty)
        .length;

    print(
      '文本: ${text.length}字符, ${text.split('\n').length}行 → ${candidates.length}题',
    );
    print(
      '题型: ${typeCounts.entries.map((e) => '${e.key}:${e.value}').join(', ')}',
    );
    ...
    for (var i = 0; i < candidates.length && i < 5; i++) {
      ...
      final t = c.title.length > 45
          ? '${c.title.substring(0, 45)}...'
          : c.title;
      print(
        '  [$i] ${c.candidateType.name} ans="${c.answer}" opts=${c.options.length}',
      );
      print('       "$t"');
    }
