...
void main() async {
  ...
  if (!await file.exists()) { print('Not found: $path'); return; }
  ...
  print('── 统计 ──');
  print('题型: ${typeCounts.entries.map((e) => '${e.key.name}:${e.value}').join(', ')}');
  ...
  print('有答案: $withAns/${candidates.length}  有解析: $withExpl/${candidates.length}  有选项: $withOpts/${candidates.length}');
  ...
  for (var i = 0; i < candidates.length && i < 20; i++) {
    ...
    print('  [$i] ${c.candidateType.name} | ans="${c.answer}" | opts=${c.options.length} | conf=${c.confidence.toStringAsFixed(2)}');
    print('      "$t"');
    if (c.explanation.isNotEmpty) {
      final e = c.explanation.length > 60 ? '${c.explanation.substring(0, 60)}...' : c.explanation;
      print('      解析: "$e"');
    }
  }
  ...
  // Check for issues
  print('\n── 问题检查 ──');
  final noAnswer = candidates.where((c) => c.answer.isEmpty && c.options.isNotEmpty).length;
  final noOptions = candidates.where((c) => c.options.isEmpty && c.candidateType == CandidateType.singleChoice).length;
